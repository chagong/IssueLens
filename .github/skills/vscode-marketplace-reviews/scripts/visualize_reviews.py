#!/usr/bin/env python3
"""
Visualize VS Code Marketplace Extension Reviews.
...
Usage:
    py visualize_reviews.py --input reviews.json --output chart.png [--title "Extension Name"] [--no-show]
"""
...
def print_summary(reviews: list, title: str = "VS Code Extension") -> None:
...
def plot_version_distribution(reviews, ax):
    """Bar chart of review count by extension version (top 10)."""
    versions = Counter(r.get("productVersion", "unknown")
                       for r in reviews)
    top = versions.most_common(10)
    if not top:
        ax.text(0.5, 0.5, "No version data", ha="center", va="center",
                transform=ax.transAxes)
        return

    labels, counts = zip(*top)
    bars = ax.barh(range(len(labels)), counts, color="#9b59b6",
    ...
    ax.set_title("Reviews by Extension Version (Top 10)", fontweight="bold")
...
def _render_individual_charts(reviews, scope, title):
    ...
    fig, ax = plt.subplots(figsize=(7, 3.5))
    plot_version_distribution(reviews, ax)
    fig.tight_layout()
    charts["version_distribution"] = fig_to_base64(fig)
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize VS Code Marketplace extension reviews")
    parser.add_argument("--input", type=str, default=None,
                        help="Input JSON file path")
    parser.add_argument("--output", type=str, default=None,
                        help="Output PNG file path")
    parser.add_argument("--title", type=str, default="VS Code Extension",
                        help="Extension name shown in chart titles")
    ...
    if scope == "long":
        ...
        plot_version_distribution(reviews, axes[1, 0])
